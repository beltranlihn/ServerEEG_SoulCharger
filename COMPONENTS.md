...
| Monitor OSC en vivo | Tres osciloscopias por panel (calma, pulso como **ECG sintético**, quietud) + cadencia real en Hz. Canvas plano, **no Chart.js** | `soul-charger-admin.html` (`SignalScope`, `ecgWave`, `drawScopes`, `updateReadouts`) | ✅ cifras y layout verificados; dibujo sin ver | R17, R18 |
| Gráfica de sesión (Chart.js) | ⚠️ **Fuera de pantalla, pero viva**: alimenta el PNG de `research/` y dispara `persistSession()`. Redibujada a 10 Hz | `soul-charger-admin.html` (`.charts-row`, `initChart`, `updateChart`, `exportChart`) | ⚠️ frágil: borrarla del DOM rompe el registro de sesiones | R18 |
